...
\begin{figure}
	...
	\caption{Representation of $\tau_z(p)=q$, or, equivalently, $t_Y(\tau_{z'})(p)=q$.
		The relative path starts in state $p$ with the head scanning the last symbol $Y$ of $z$, performs a computation on the left part of the tape ending with $Y$, and eventually passes it to the right in state $q$.}
	\label{fig:transtab}
\end{figure}
