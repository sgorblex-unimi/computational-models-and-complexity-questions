...
\begin{proof}
	...
	\begin{description}
		...
		\item[Step] Given $x\in\Sigma\star$ and $r\in Q$, assume $\delta'(q'_I,x)=(\tau_x,r)$ if and only if there exists a computation of $A$ on input $x$ that passes the last symbol of $x$ for the first time in state $r$, and let the tape contain $w=x\sigma$, with $\sigma\in\Sigma$. The computation path is represented in \Cref{fig:transtabproof} $(1)$.
		      \begin{description}
