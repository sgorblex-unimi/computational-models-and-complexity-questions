...
% TODO: add picture with visuals for the terminology
...
\begin{lemm}
	For all $w\in\Sigma\star$, the first component of $\delta(q'_I,w)$ is $\tau\in\transtabset$ if and only if $\tau=\tau_w$.
\end{lemm}

\begin{lemm}\label{lem:transtab2DFA}
	For each $w\in\Sigma\star$ and $p\in Q$, $\delta'(q'_I,w)=(\tau_w,p)$ if and only if there exists a computation of $A$ on input $w$ that passes the last symbol of $w$ for the first time in state $p$.
\end{lemm}
\begin{proof}
	By induction on $w$:
	\begin{description}
		...
		\item[Step] Given $x\in\Sigma\star$ and $r\in Q$, assume $\delta'(q'_I,x)=(\tau_x,r)$ if and only if there exists a computation of $A$ on input $x$ that passes the last symbol of $x$ for the first time in state $r$, and let the tape contain $w=:x\sigma$, with $\sigma\in\Sigma$.
		      \begin{description}
			      \item[$\Rightarrow$)] Assume $\delta'(q'_I,w)=(\tau_w,p)$.
			            ...
			            By definition of transition table, there is a computation path that starts on $\sigma$ and first passes it to the right in state $p$.
			            By combining the two computations we get the desideratum.
			      \item[$\Leftarrow$)] Assume that there exists a computation of $A$ on input $w=x\sigma$ that passes $\sigma$ for the first time in state $r$.
			            Such computation can be split in two parts, the first ending when $\sigma$ is first read in a state $r$.
...
\begin{proof}
	...
	\begin{description}
		\item[$\genlang(A')\subseteq\genlang(A)$:] If $\delta'(q'I_,w)=(\tau_w,p)$ and $(\tau_w,p)$ is final, by definition $t_\rem(\tau_w)(p)\in F$, therefore $A$ on input $w$ finishes its computation by passing $\rem$ in a final state.
...
\begin{thrm}
	Every $n$-state 2DFA can be simulated by a 1DFA with an exponential number of states in $n$.
\end{thrm}
