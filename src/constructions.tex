% TODO: mention appendix in proofs throughout the chapters
...
In some constructions, it is more intuitive to build a set of initial states for a \NFA rather than a single state.
...
Note that this is only possible on \NFAs, since the removal of multiple starting states from a \DFA can imply an exponential growth in the number of states (\cite{TODO}).
...
\begin{proof}
	...
	\end{align*}
\end{proof}
As a concluding remark, we note that a similar construction could be done by adding $\emptyword$-transitions originating in the new initial state.
However, removing $\emptyword$-transitions from a \NFA is generally costly (exponential in the worst case), so we prefer this more direct simulation \cite{TODO}.
...
Here we present a construction that proves that on a \NFA such simplification has no additional cost in the number of states in case the prefix or suffix is over a separate alphabet with respect to the original language.
\begin{thrm}
...
As said above, since removing $\emptyword$-transitions from a \NFA can determine an exponential growth in size, we prefer to avoid such strategy.
...
This theorem proves that the obtained \NFA can be converted in one that accepts $L$ with no additional cost in the number of states.



\section{\NFA for the reversal}
The next construction, originally introduced by \citeauthor{RabSco59} (\cite{RabSco59}), lets us build, from a \NFA accepting a language, a \NFA for its reversal, with no growth in size.
\begin{thrm}
	For every $n$-state \NFA $A$, there exists an $n$-state \NFA accepting $\genlang(A)\rev$.
\end{thrm}
\begin{proof}
	Let $A:=(Q,\Sigma,\delta,q_I,F)$ a \NFA.
	Define the \NFA $A':=(Q,\Sigma,\delta',I,F')$, where
	\begin{itemize}
