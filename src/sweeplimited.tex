...
We therefore prove a lower bound on the number of states of an \ONFA simulating a sweeping \kDLA, hence showing that \kDLAs can be exponentially more succinct.

Consider the following language, parameterised by a positive integer $n$:
\begin{equation}\label{eq:lowersweep}
	L_n := \set{w F_1\cdots F_n \mid w\in\set{a,b}\star, F_1,\dots,F_n\in\set{1,\dots,n}, w_{F_1\cdots F_n}\rev=a} \text.
\end{equation}
The language extends the classic witness language family where the $m$-th symbol from the right is $a$, making the calculation of $m$ part of the computation.
In particular, for each word $m$ is defined as the least common multiple of factors $F_1,\dots,F_n$ written as an additional suffix appended to the word.


\subsection{A \kDLA accepting \texorpdfstring{$L_n$}{Ln}}
Chosen a value for $n$, we describe a \DLA{2n} that behaves as follows. Given a word $wF_1\cdots F_n$ (as described in (\ref{eq:lowersweep})):
\begin{enumerate}
	\item on the first left-to-right traversal, the automaton saves the factor $F_1$ in the finite state memory and deletes it from the tape by replacing it with a special symbol $X$, while leaving everything else unaltered;\footnote{%
		      Technically, because of how $k$-limited automata are defined, all transitions in the first $k$ visits are rewriting. For simplicity, we may say that a cell remain unaltered, while its symbol is actually overwritten with a symbol in $\Gamma_1$ which keeps the original information of the one in $\Sigma$.}
	\item on the following sweep to the left, it deletes all the symbols in $w\rev$ that are in a position that is not a multiple of $F_1$, by substituting them with another special symbol $Y$;
	\item the automaton then proceeds as in the first two steps for all others factors, making sure to only count symbols different from $Y$ when deleting (notice that the cells containing what was originally $w\rev$ can be located even after overwriting since their symbols are overwritten with $Y$ and not $X$);
	\item eventually the automaton will perform a traversal to the right which will not find any more factors. In the following sweep, the automaton checks that the first symbol different from $X$ and $Y$, which is in the position of $w\rev$ that is multiple of all the factors, is $a$;
	\item with a final sweep to the right, the automaton checks that there are exactly $n$ occurrences of $X$, that is, the number of what were originally factors is $n$.
\end{enumerate}
The number of necessary rewrite-capable visits is $2n$, since each pair of sweeps takes care of one factor (the final traversals do not need to write).
The automaton needs a few counters: one for each possible factor and one for counting the occurrences of $X$ in the final step.
However, no two counters are used at the same time, therefore the number of states is additive:
\begin{equation*}
	1+\dots+n+n = \frac{n(n+1)}{2}+n = O(n^2) \text.
\end{equation*}
Considering that the direction of movement can be deduced by the symbols under the tape head (specifically by the parity of $i$ of the set $\Gamma_i$ to which they belong), a constant number of states is sufficient for the starting and accepting phases.
The size of the working alphabet is linear in $n$, since only $2$ extra symbols per write are required along the ones leaving $a$ and $b$ intact (the number is linear and not constant since the number of rewritings depends on $n$).
Therefore, overall the size of the machine is quadratic in $n$.

\subsection[Size of finite state automata accepting \texorpdfstring{$L_n$}{Ln}]{Lower bounds on the size of finite state automata accepting $L_n$}
A \ODFA of size double exponential in $n$ can accept $L_n$ by saving in the finite state memory a window of the last $n^n$ symbols, and then

In order to prove that each \ODFA needs a double exponential number of states in $n$ in order to accept $L_n$, we use a standard distinguishability argument.
...
Then, the string $z:=b^{i-1} n^n$ (where $n^n$ is the string made by $n$ repetitions of the symbol $n$) distinguishes $x$ and $y$.
For example, if $x_i=a$ and $y_i=b$, the $n^n$-th symbol of $(xz)\rev$ is $a$, meaning $xz\in L_n$, while the $n^n$-th symbol of $(yz)\rev$ is $b$, meaning $yz\notin L_n$.

Because $\card S=2^{n^n}=2^{2^{n\log n}}$, every \ODFA accepting $L_n$ needs a number of states two times exponential in $n$.
...
The lower bound above can be considered "weak", since it requires a different value of $k$ for a \kDLA accepting $L_n$ for each value of $n$.
A stronger bound would consist in finding a language $L'_n$ and a constant value of $k$ for which there exists a \kDLA accepting $L'_n$ while a \ODFA requires a double exponential number of states in $n$, for each value of $n$.
