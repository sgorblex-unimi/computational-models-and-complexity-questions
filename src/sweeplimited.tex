...
\subsection{A \kDLA accepting \texorpdfstring{$D_n$}{Ln}}
...
\begin{enumerate}
	...
	\item after $n$ deleting sweeps ($2n$ sweeps in total), the automaton checks that the first non-deleted symbol is $a$.
...
Considering that the direction of movement and the current sweep can be deduced from the scanned symbols (specifically from the parity and the index of $i$, respectively, if the current symbol belongs to $\Gamma_i$), only an accepting state is needed for the rest of the computation.
The total number of states is thus $n+1$.
The size of the working alphabet is also linear in $n$, since only one extra symbol per write is required along the ones leaving $a$ and $b$ intact (the number is linear and not constant since the number of rewritings depends on $n$).
...
The lower bound above can be considered "weak", since it requires a different value of $k$ for a \kDLA accepting $D_n$ for each value of $n$.
A stronger result would consist in finding a language $L'_n$ and a constant value of $k$ for which there exists a \kDLA accepting $L'_n$ while a \ODFA requires a double exponential number of states in $n$, for each value of $n$.
