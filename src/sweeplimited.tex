...
\begin{defn}[matching crossing sequences for \kDLA]
...
\begin{proof}
	...
	An equivalent \ONFA with a single starting state can be obtained by only adding one state, as described in \Cref{sub:multistart}.
\end{proof}
...
\subsection{A sweeping \texorpdfstring{\kDLA}{k-DLA} accepting \texorpdfstring{$D_n$}{Ln}}
