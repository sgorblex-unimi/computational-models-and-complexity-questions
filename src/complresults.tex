...
\begin{itemize}
	...
	\item The cells highlighted in \hlB{blue} consist in a relaxed version of the problem, with an extension of \TNFAs, namely \ODLAs, as the simulating machine.
	\item The cells highlighted in \hlG{green}%
	      \easteregg{\marginpar{"Like a froggy ferny cabbage"}}
...
\begin{itemize}
	...
	\item $\ge\Cdexp$: distinguishability arguments on witness languages: the block language~$L_n$ defined by \triplecite{PigPis14}%
	      \easteregg{\marginpar{The "(Tape) Cell Block Tango"}}
	      and the "reset" witness languages by \triplecite{PigPri+22}.
\end{itemize}
...
\begin{itemize}
	...
	\item $\ge\Cexp$: since the unary witness language~$\set{a^{2^n}}\star$ is recognized by the linear size \ODLA presented by \triplecite{PigPri19}, but cannot be recognized by a \TNFA with less than~$\Cexp$ states, as proven by \triplecite{MerPig00}.
\end{itemize}
...
\begin{itemize}
	\item $\le\Cdexp$: by using the transition table construction for general \OLAs presented by \triplecite{PigPis14},
...
\begin{itemize}
	\item $\le\Cexp$: by using the transition table construction for general \OLAs presented by \triplecite{PigPis14},
...
\begin{itemize}
	\item $\le\Cexp$: by using the transition table construction for general \ODLAs presented by \triplecite{PigPis14},
	\item $\ge\Cexp$: since the witness language~$K_n$ is recognized by a linear size \OMODLA, but cannot be recognized by a \TNFA with less than~$\Cexp$ states, as proven by \triplecite{PigPri23a}.
\end{itemize}
...
\begin{itemize}
	...
	\item $\ge\Cexp$: since the witness language~$\double:=\set{ww\mid w\in\set{a,b}^n}$ is recognized by a linear size \OMODLA, but cannot be recognized by a \ONFA with less than~$\Cexp$ states (proven in \Cref{thm:OM1DLAto1NFAlower}).
\end{itemize}
...
For the sake of our usage of the function, the following approximation, which was found by Landau himself in the early twentieth century, is sufficient~\cite{Lan03,Lan09}:\footnote{%
	The most accurate estimate known to date is~$F(n) = e^{(1+o(1))\sqrt{n\ln n}}$, found by \triplecite{Gef07}.}
\begin{equation}
	F(n) = \CsubEq \text.
\end{equation}
...
\begin{itemize}
	...
	\item $\ge\Cexp$: since the witness language~$\set{a^{2^n}}\star$ is recognized by the linear size \ODLA presented by \triplecite{PigPri19}, but cannot be recognized by a \TNFA with less than~$\Cexp$ states, as proven by \triplecite{MerPig00},
...
\begin{itemize}
	\item $\le O(n^3)$: by using the simulation via computation trees for general \OMODLAs presented by \triplecite{PigPri23a}.
\end{itemize}
\paragraph{\OMODLA{}\tto\TNFA}
\begin{itemize}
