...
The current known bounds for the simulations between unary once-marking $1$-limited automata and classical regular language acceptors are summarized in \Cref{tab:sims-om-unary-oncemarking}.
...
The bounds derive from either unary $1$-limited automata results, general-case once-marking $1$-limited automata results, or results on classical unary language acceptors.
Because in the latter some subexponential bounds appear, some of the non-tight bounds have a less than exponential distance (subexponential to exponential or polynomial to subexponential).
An explanation of each bound is given in \Cref{app:results}.

Working with subexponential bounds can be challenging, since, unlike with exponentials and polynomials, the operations between different functions (product, composition, etc.) can produce another subexponential function or an exponential one, depending on the specific functions.
For example, by combining the subexponential construction to simulate a \ONFA with a \ODFA with the construction to simulate a \OLA with a \ONFA, one might expect to obtain a cost that is less than double exponential, but because of the specific functions this is not the case.
The simulation of \OMOLA by \ODFA is particularly interesting as, as it was the case for general $1$-limited automata, many results can be obtained as consequences of an improved simulation or a witness language that implies a tighter lower bound.
We investigated both these paths, and we conjecture that, in the unary case, a better upper bound can be found.



\section{Improving known bounds}\label{sec:oncemarking-ideas}
In order to research the possibility of improving the known bounds on the simulation of once-marking $1$-limited automata by finite state automata, we considered techniques from both the unary and the regular language world.
...
Unfortunately, the techniques we know to produce lower bounds for bounded regular languages typically define $k$ in function of the parameter under consideration, where we would need a constant $k=2$.
An example is the "tool for constructing lower bound witnesses" proposed by \citeauthor{HerKut+17} \cite{HerKut+17}.
...
In particular, the concatenation of two similar (but different) witness languages might force a \ODFA to need a subexponential component for each part of the input word, while a once-marking automaton could simply nondeterministically guess and mark the first symbol of the second part and verify separately that the two substrings belong to their respective language.
...
\paragraph{Limiting the marking position} A promising idea is the one of restricting the positions where the automaton can mark cells, specifically by limiting the distance from the end-markers where the writing transition is performed.
The idea comes from the simulation of unary \TNFA by \ODFA presented by \triplecite{MerPig01}.
...
We believe that a similar argument can be made to limit the positions where, in a once-marking $1$-limited automaton, the write operation is performed, thus reducing the size of a simulating machine.
We point out that if a subexponential simulation of \OMOLA by \ODFA was to be found, all the bounds on one-way finite automata would collapse to subexponential.
